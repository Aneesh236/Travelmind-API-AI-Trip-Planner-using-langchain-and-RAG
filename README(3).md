...
The source code is available in this repository. A public live-demo link will
be added after deployment to a Python hosting service.

## RAG architecture

```mermaid
flowchart TD
    A[Curated Kerala knowledge] --> B[LangChain text chunks]
    B --> C[OpenAI embeddings]
    C --> D[(ChromaDB vector store)]
    E[Traveller preferences] --> F[Semantic search]
    D --> F
    F --> G[Retrieved travel context]
    G --> H[OpenAI chat model]
    H --> I[Grounded itinerary and sources]
```
...
GitHub stores and presents the complete source code, documentation and project
history. Because this is a Python application, GitHub Pages alone cannot run
the Gradio backend. Run it locally or connect this repository to a Python
hosting platform for a live demo.

Before publishing, confirm that `.env` is ignored and that no API key appears
